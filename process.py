...
def convert_to_alpaca_format(df):
    ...
    for _, row in df.iterrows():
        ...
        1. 主线 (核心故事线): 评估主线故事的质量和潜力。
            - 通过标准: 主线 **描述清晰且集中**, 核心矛盾突出且简单易懂, 但矛盾不宜复杂；剧情 **节奏紧凑**，符合短剧的格式要求；逻辑 **自洽**；主线具备支撑剧情发展和吸引观众的基础，
            - 不通过标准: 主线 **矛盾分散**，缺乏明确的冲突焦点；或者 剧情 **节奏拖沓**，不符合短剧的紧凑要求；或者 **逻辑漏洞明显**，难以支撑剧情发展。
        ...
        try:
            ...
            if raw_output_data is None:
                continue
            output_data = json.loads(raw_output_data)
            ...
            print(f'Failed to parse {raw_output_data}')
            continue  # 跳过解析失败的行
...
def main():
    ...
    jsonl_file_path = 'script_review_part_2_alpaca.jsonl'  # 输出 JSONL 文件
    # 读取数据
    df = read_csv_data(csv_file_path)
    # 转换数据
    alpaca_data = convert_to_alpaca_format(df)
    # 保存为 jsonl 文件
    save_to_jsonl(alpaca_data, jsonl_file_path)
